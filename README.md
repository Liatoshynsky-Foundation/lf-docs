# Liatoshynsky Foundation Documentation

This repository serves as the central source of truth for all our technical documentation across all projects and repositories. By keeping our documentation in one dedicated place, we ensure it remains organized, accessible, and easy to maintain.

## Documentation Directory

Explore the available documentation below:

### Admin Panel
* **[Admin Theme Guide](./docs/admin-panel/theme/README.md)** — Comprehensive guide on our custom Material-UI theme, component overrides, color dictionaries, and typography variants.

*(More sections will be added here as the project evolves.)*

## How to Contribute

Follow the contribution and source control process defined in [CONTRIBUTING.md](CONTRIBUTING.md), including guidelines for repository structure, file naming, branching, pull requests, reviews, and other documentation policies.

If you are new to Git, GitHub, or Git Bash, see the [Git documentation contribution cheat sheet](git-contribution-cheat-sheet.md). It provides a beginner-friendly, step-by-step workflow with the Git commands needed to create a branch, make and review changes, commit and push them, create a pull request, and synchronize your local repository after the changes are merged.

---
