...
Each image placeholder has a predefined aspect ratio. Administrators can reposition and resize the display area within that ratio but cannot change the placeholder proportions.
...
Changes made in the admin panel remain in the working copy and become visible on the public website only after the corresponding language version (page) is published.

## Image assets and display area resizing
...
The same Image Asset can have different crops depending on where and how it is used. In particular, Ukrainian and English versions of a page can use the same image with independent display area.

Changing the display area for one language version does not affect the other language version.
...
- **Редагувати зображення** — adjust the display area of the currently selected image.
...
Language badges identify the corresponding version. If the same image has different display areas in Ukrainian and English, each version is displayed separately.

Selecting an image from another language version reuses both the image and its current display area. The administrator can then keep the existing display area or adjust it before applying the image.

### Галерея

Displays public Image Assets available in the media library.

Administrators can search and sort available images and select one for the current placeholder. After selection, the image opens in the display area resizing dialog.
...
After an image is selected, it opens in the display area resizing dialog.

## Resizing image display area 

Selecting **Редагувати зображення**, or choosing a new image, opens the display area resizing dialog.

The dialog displays the image and a frame with the predefined aspect ratio of the current placeholder. Administrators can resize and reposition the frame without changing its aspect ratio.

Available actions:

- **Застосувати** — save the display area for the current page, language, and placeholder and return to the page editor.
...
Ukrainian and English pages maintain independent image display areas.

Administrators can reuse an image and display area from another language version through the **Використані** tab. Selecting an image marked with another language initializes the display area resizing dialog with that version's display area.

The administrator can apply the display area without changes or adjust it for the current language version.

Subsequent changes remain independent. Updating the display area in one language version does not automatically update the other.
